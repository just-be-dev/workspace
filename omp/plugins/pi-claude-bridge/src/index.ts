import { calculateCost, type AssistantMessage, type AssistantMessageEventStream, type Context, type Model, type SimpleStreamOptions, type Tool } from "@oh-my-pi/pi-ai";
import * as piAi from "@oh-my-pi/pi-ai";
import { getBundledModels as getModels } from "@oh-my-pi/pi-catalog/models";
import { buildSessionContext, keyHint, type CompactionEntry, type ExtensionAPI, type ExtensionUIContext } from "@oh-my-pi/pi-coding-agent";
...
export default function (pi: ExtensionAPI) {
	...
	// OMP no longer exports the legacy `compact` helper that the Pi build uses
	// to take over compaction. Let OMP handle compaction natively, then rebuild
	// the Claude Code session from the post-compaction transcript below.
	...
	if (askConf?.enabled !== false) {
		const { z } = pi.zod;
		const modeSchema = allowFull ? z.enum(["read", "full", "none"] as const) : z.enum(["read", "none"] as const);
		const askClaudeParams = z.object({
			prompt: z.string().describe("The question or task for Claude Code. By default Claude sees the full conversation history. Don't research up front, let Claude explore."),
			mode: modeSchema.optional().describe(modeDesc),
			model: z.string().optional().describe('Claude model (e.g. "opus", "sonnet", "haiku", or full ID). Defaults to "opus".'),
			thinking: z.enum(["off", "minimal", "low", "medium", "high", "xhigh"] as const).optional().describe("Thinking effort level. Omit to use Claude Code's default."),
			isolated: z.boolean().optional().describe("When true, Claude sees only this prompt (clean session). When false (default), Claude sees the full conversation history."),
		});
		const registerAskClaudeTool = (name: string, label: string, description: string) => {
			pi.registerTool({
				name,
				label,
				description,
				...
				renderCall(args, _options, theme) {
					let text = theme.fg("mdLink", theme.bold(`${name} `));
		...
		const configuredName = askConf?.name ?? "AskClaude";
		const configuredLabel = askConf?.label ?? "Ask Claude Code";
		const configuredDescription = askConf?.description ?? (allowFull ? DEFAULT_TOOL_DESCRIPTION_FULL : DEFAULT_TOOL_DESCRIPTION);
		registerAskClaudeTool(configuredName, configuredLabel, configuredDescription);
		if (configuredName === "AskClaude") {
			registerAskClaudeTool(
				"ask_claude",
				`${configuredLabel} (API alias)`,
				`${configuredDescription}\n\nAlias for AskClaude for clients that only surface snake_case tool names.`,
			);
		}
	}
}

---
name: effect-setup
description: Bootstrap or align an Effect 4 project to the house setup — Effect 4 RC deps, tsgo + effect-tsgo patch, @effect/language-service tsconfig plugin, and the effect-solutions consultation rule. Use when a project needs Effect installed for the first time, when migrating from Effect 3 to Effect 4, or when adding tsgo config to an existing Effect 4 project.
...
- **Effect 4 RC** (`effect@rc`, `@effect/platform-*`) — not Effect 3 stable and not the `latest` dist-tag while v4 is on `rc`.
...
Do **not** run unprompted on a project that already has Effect 3 stable working — migrating to Effect 4 is a deliberate choice the user must opt into.
...
If the project already has `effect@3.x` installed, ask before bumping to Effect 4. For greenfield projects with nothing in place, just proceed.

### Step 3 — Apply each piece

#### 3a. `package.json` — Effect 4 RC deps + scripts

Target shape (recommend pulling the `rc` dist-tag for Effect 4 while v4 is in release candidate):
...
For Bun/Node platform packages, use the matching Effect 4 RC platform packages:
...
Atom/AtomRegistry live under `effect/unstable/reactivity/` in Effect 4 — that's not a typo, the API is still stabilizing.
...
| File | Purpose |
|------|---------|
| `package.json` | Effect 4 RC deps + `typecheck` / `prepare` scripts |
